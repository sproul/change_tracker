...
class Json_change_tracker
        ...
        def go(json_text)
                ...
                if !h
                        return usage("trouble parsing \"#{json_text}\": #{emsg}")
                end
                begin
                        self.op = get(h, "op")
                        case self.op
                        ...
                        when "list_files_changed_between"
                                ...
                                return usage("did not know how to interpret op '#{op}'")
                        end
                rescue User_error => e_obj
                        return usage(e_obj.emsg)
                end
                z = nil
                x.each do | elt |
                        if !z
        ...
        class << self
                ...
                def test_assert_result_from_json(json, expected_result, title=nil)
                        actual_result = Json_change_tracker.new.go(json)
                        if !title
                                title = "from #{json}"
                        end
                        U.assert_json_eq(expected_result, actual_result, title)
                end
                def test_error_result_from_json(json, expected_result, title=nil)
                        actual_result = Json_change_tracker.new.go(json)
                        if !title
                                title = "from #{json}"
                        end
                ...
                def test_bad_json()
                        ...
                        test_error_result_from_json(%Q[{ "op" : "some_nonexistent_op" }], "did not know how to interpret op 'some_nonexistent_op'\nExample usage:\n#{Json_change_tracker.usage_full_examples}\n@@ example of list_bug_IDs_between\n@@ example of list_changes_between\n@@ example of list_files_changed_between\n", "nonexistent op")

                        test_error_result_from_json("whatever", "trouble parsing \"whatever\": 757: unexpected token at 'whatever'\nExample usage:\n#{Json_change_tracker.usage_full_examples}", "ridiculous null request")
                        test_error_result_from_json(%Q[{ "op" : "list_bug_IDs_between" }], "did not find anything for key 'cspec1'\nExample usage:\n@@ example of list_bug_IDs_between\n", "no cspec1")
                        test_error_result_from_json(%Q[{ "op" : "list_bug_IDs_between", "cspec1" : "#{cspec1}" }], "did not find anything for key 'cspec2'\nExample usage:\n@@ example of list_bug_IDs_between\n", "no cspec2")
                        test_error_result_from_json(%Q[{ "op" : "list_changes_between" }], "did not find anything for key 'cspec1'\nExample usage:\n@@ example of list_changes_between\n", "no cspec1 1")
                        test_error_result_from_json(%Q[{ "op" : "list_changes_between", "cspec1" : "#{cspec1}" }], "did not find anything for key 'cspec2'\nExample usage:\n@@ example of list_changes_between\n", "no cspec2 3")
                        test_error_result_from_json(%Q[{ "op" : "list_files_changed_between" }], "did not find anything for key 'cspec1'\nExample usage:\n@@ example of list_files_changed_between\n", "no cspec1 3")
                        test_error_result_from_json(%Q[{ "op" : "list_files_changed_between", "cspec1" : "#{cspec1}" }], "did not find anything for key 'cspec2'\nExample usage:\n@@ example of list_files_changed_between\n", "no cspec2 4")
                end
