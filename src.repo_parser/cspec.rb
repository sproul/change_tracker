class Cspec < Error_holder
        ...
        def list_changed_files()
                self.repo.vcs.get_changed_files_array(self.commit_id)
        end
        def list_files_changed_since(other_commit)
                ...
                commits.each do | commit |
                        changed_file_list.concat(commit.list_changed_files())
                end
        ...
        class << self
                ...
                def from_s(s, arg_name="Cspec.from_s")
                        ...
                        if Cspec.is_repo_and_commit_id?(s)
                                ...
                                if repo_and_commit_id !~ /(.*);([0-9a-fA-F]+)$/
                ...
                def from_repo_and_commit_id(repo_and_commit_id, comment=nil)
                        ...
                        repo_spec, commit_id = $1, $2
                        raise "unexpected lack of repo in #{repo_and_commit_id}" unless repo_spec
                ...
                def is_repo_and_commit_id?(s)
                        ...
                        r = Repo.from_spec(s, false)
                        if r && r.commit_id
                                puts "is_repo_and_commit_id?(#{s}): yes"
                                true
                        else
                                puts "is_repo_and_commit_id?(#{s}): no"
                                false
                        end
                end
                ...
                def test()
                        ...
                        U.assert_eq(false, Cspec.is_repo_and_commit_id?("git;git.osn.oraclecorp.com;osn/serverintegration"), "Cspec.is_repo_and_commit_id (no commit ID) 3")
